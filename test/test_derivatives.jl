...
# Simple tests of derivatives with respect to variables
for i=1:3

    @test isapprox(
        evalDiffTreeArray(tree, input_matrix, options, i)[2][1],
        true_grad[i],
        rtol=1e-5
    )

    @test isapprox(
        evalGradTreeArray(tree, input_matrix, options; variable=true)[2][i, 1],
        true_grad[i],
        rtol=1e-5
    )
end