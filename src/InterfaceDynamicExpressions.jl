...
"""
    eval_tree_array(tree::AbstractExpression, X::AbstractArray, options::Options; kws...)
...
# Arguments
- `tree::AbstractExpression`: The root node of the tree to evaluate.
...
function eval_tree_array(
    tree::AbstractExpression, X::AbstractMatrix, options::Options; kws...
)
...
function expected_array_type(::AbstractExpression, X::AbstractArray, ::OperatorEnum)
    return typeof(similar(X, axes(X, 2)))
end

"""
    eval_diff_tree_array(tree::AbstractExpression, X::AbstractArray, options::Options, direction::Int)
...
- `tree::AbstractExpression`: The expression tree to evaluate.
...
function eval_diff_tree_array(
    tree::AbstractExpression, X::AbstractArray, options::Options, direction::Int
)
    return eval_diff_tree_array(tree, X, options.operators, direction)
end

"""
    eval_grad_tree_array(tree::AbstractExpression, X::AbstractArray, options::Options; variable::Bool=false)
...
function eval_grad_tree_array(
    tree::AbstractExpression, X::AbstractArray, options::Options; kws...
)
    return eval_grad_tree_array(tree, X, options.operators; kws...)
end

"""
    differentiable_eval_tree_array(tree::AbstractExpression, X::AbstractArray, options::Options)
...
"""
    string_tree(tree::AbstractExpression, options::Options; kws...)

Convert an equation to a string.

# Arguments

- `tree::AbstractExpression`: The equation to convert to a string.
...
"""
    print_tree(tree::AbstractExpression, options::Options; kws...)
...
- `variable_names::Union{Array{String, 1}, Nothing}=nothing`: what variables
    to print for each feature.
"""
function print_tree(tree::AbstractExpression, options::Options; kws...)
    return print_tree(tree, options.operators; kws...)
end
function print_tree(io::IO, tree::AbstractExpression, options::Options; kws...)
    return print_tree(io, tree, options.operators; kws...)
end

"""
    convert(::Type{<:AbstractExpression{T}}, tree::AbstractExpression, options::Options; kws...) where {T}

Convert an equation to a different base type `T`.
"""
function Base.convert(
    ::Type{N}, tree::AbstractExpression, options::Options
) where {T,N<:AbstractExpression{T}}
...
`AbstractExpression` type. While by default this is already done for operators defined
...
function (tree::AbstractExpression)(X, options::Options; kws...)
...
function DynamicExpressions.EvaluationHelpersModule._grad_evaluator(
    tree::AbstractExpression, X, options::Options; kws...
)
