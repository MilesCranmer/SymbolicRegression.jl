...
function s_r_cycle(
    ...
    batched_dataset = @take(options.batching) ? batch(dataset, options.batch_size) : dataset
...
function optimize_and_simplify_population(
    ...
    do_optimization = rand(pop.n) .< @take(options.optimizer_probability)
    ...
    should_thread =
        !@take(options.deterministic) && !(isa(@take(options.autodiff_backend), AutoEnzyme))
    ...
    @threads_if should_thread for j in 1:(pop.n)
        if @take(options.should_simplify)
            ...
            tree = simplify_tree!(tree, @take(options.operators))
            tree = combine_operators(tree, @take(options.operators))
            pop.members[j].tree = tree
        end
        if @take(options.should_optimize_constants) && do_optimization[j]
