...
function BINOP!(x::AbstractVector{T}, y::AbstractVector{T}, ::Val{i}, options::Options) where {i,T<:Real}
    op = options.binops[i]
    # broadcast!(op, x, x, y)
    @inbounds @simd for j=1:size(x, 1)
        x[j] = op(x[j], y[j])
    end
end

function UNAOP!(x::AbstractVector{T}, ::Val{i}, options::Options) where {i,T<:Real}
    op = options.unaops[i]
    @inbounds @simd for j=1:size(x, 1)
        x[j] = op(x[j])
    end
...
function evalTreeArray(tree::Node, cX::AbstractMatrix{T}, options::Options)::Tuple{AbstractVector{T}, Bool} where {T<:Real}
    ...
    if tree.degree == 1
        op_idx = tree.op
        UNAOP!(cumulator, Val(op_idx), options)
    else
        ...
        op_idx = tree.op
        BINOP!(cumulator, array2, Val(op_idx), options)
    end
