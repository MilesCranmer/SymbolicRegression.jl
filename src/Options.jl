...
@unstable @save_kwargs DEFAULT_OPTIONS function Options(;
    ...
    ### Missing parameters that need explicit declaration:
    fraction_replaced::Union{Nothing,Real}=nothing,
    fraction_replaced_hof::Union{Nothing,Real}=nothing,
    topn::Union{Nothing,Integer}=nothing,
    optimizer_probability::Union{Nothing,Real}=nothing,
    optimizer_algorithm::Union{Nothing,Optim.AbstractOptimizer}=Optim.BFGS(linesearch=LineSearches.BackTracking()),
    ...
    optimizer_options::Union{Optim.Options,NamedTuple,Nothing}=nothing,
    complexity_mapping::Union{Nothing,ComplexityMapping}=nothing,
    perturbation_factor::Union{Nothing,Real}=nothing,
    probability_negate_constant::Union{Nothing,Real}=nothing,
